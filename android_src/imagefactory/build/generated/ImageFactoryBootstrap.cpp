...
#define TAG "ti.imagefactory"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> ImageFactory_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("ImageFactory.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ImageFactoryBindings::lookupGeneratedInit(
...
static void ImageFactory_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(ImageFactory_getBinding)->GetFunction());
}

static void ImageFactory_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ImageFactoryBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ImageFactoryBinding = {
	"ti.imagefactory",
	ImageFactory_init,
	ImageFactory_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_imagefactory_ImageFactoryBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.imagefactory", &ImageFactoryBinding);
	titanium::KrollBindings::addExternalLookup(&(::ImageFactoryBindings::lookupGeneratedInit));
}
