...
Omadi.service.sendCommentOnLoad = function(e){"use strict";
    var subDB, dialog, json, nameTable, dir, file, string;
    ...
    try{
        if (this.responseText !== null && this.responseText !== "null" && this.responseText !== "" && this.responseText !== "" && isJsonString(this.responseText) === true) {
            ...
            try{
                ...
                Ti.API.debug("Exception at comment sync: " + ex);
                Omadi.service.sendErrorReport("Exception at json comment data sync: " + ex);
            }
            ...
            dialog.addEventListener('click', function(e) {
                try{
                    ...
                    Omadi.service.sendErrorReport("exception on logstatus comment logout: " + ex);
                }
            });
        }
        ...
        Omadi.service.sendErrorReport("Exception in update comment onload: " + ex1);
    }
};

Omadi.service.sendCommentOnError = function(e){"use strict";
    var dialog, db;
    try{
        ...
        if (this.status == 500) {
            ...
            try{
                Omadi.service.sendErrorReport('Showed the user a comment network error dialog on send: ' + this.status + " " + e.error);
            }
            catch(none2){}
        ...
        Omadi.service.sendErrorReport("Exception with update comment onerror callback: " + ex);
    }
};
...
Omadi.service.sendCommentRetries = 0;
Omadi.service.lastSendComments = 0;
Omadi.service.sendComments = function() {"use strict";
    /*jslint eqeq: true*/
    var isSendingData, http, secondsLeft, origAppStartMillis, currentWinStartMillis, windowURL, timestamp, commentOutput;
    ...
    if(typeof Ti.UI.currentWindow.appStartMillis !== 'undefined'){
        ...
        if(isNaN(origAppStartMillis) || isNaN(currentWinStartMillis)){
            Ti.API.error("comment start millis is NaN: " + currentWinStartMillis + " - " + origAppStartMillis);
            Omadi.service.sendErrorReport("comment start millis is NaN: " + currentWinStartMillis + " - " + origAppStartMillis);
        }
        else{
            if(origAppStartMillis == 0 || currentWinStartMillis == 0){
                Ti.API.error("AppStartMillis upload was zero: " + origAppStartMillis + " - " + currentWinStartMillis);
                Omadi.service.sendErrorReport("AppStartMillis comment was zero: " + origAppStartMillis + " - " + currentWinStartMillis);
            }
            else{
                if(origAppStartMillis != Ti.UI.currentWindow.appStartMillis){
                    if(Ti.App.isAndroid){
                    ...
                    //else{
                        Omadi.service.sendErrorReport("An extra iOS send comment event is being REJECTED: " + Ti.UI.currentWindow.appStartMillis + " - " + origAppStartMillis);
                        return;
                    //}
                }
            }
        ...
        Omadi.service.sendErrorReport("AppStartMillis comment was undefined, url: " + windowURL);
    }
    
    Ti.API.error("Sending Comments Now");
    timestamp = Omadi.utils.getUTCTimestamp();
    
    if((timestamp - Omadi.service.lastSendComments) < 2){
        // Do not send updates within 2 seconds of each other
        Ti.API.error("Not allowing comment send - too soon after previous send.");
        return;
    }
    
    Omadi.service.lastSendComments = timestamp;
    ...
    if (Ti.Network.online) {
        ...
        commentOutput = Omadi.service.getUpdatedCommentJSON();
        
        if(commentOutput){
            if (isSendingData) {
                
                if(Omadi.service.sendCommentRetries < 10){
                    setTimeout(Omadi.service.sendComments, 1000);
                    
                    secondsLeft = 10 - Omadi.service.sendCommentRetries;
                    if(secondsLeft < 0){
                        secondsLeft = 0;
                    }
                    
                    Ti.App.fireEvent("sendingData", {
                        message : 'Waiting to send comments... ' + secondsLeft
                    });
                }
                else{
                    Omadi.service.sendUpdateCommentRetries = 0;
                    isSendingData = Omadi.service.setSendingData(false);
                    Omadi.service.sendComments();
                }
                Omadi.service.sendCommentRetries ++;
            }
            else{
                ...
                http.open('POST', Omadi.DOMAIN_NAME + '/js-sync/omadi_comment.json');
    
                http.setRequestHeader("Content-Type", "application/json");
                Omadi.utils.setCookieHeader(http);
                
                http.onload = Omadi.service.sendCommentOnLoad;
                http.onerror = Omadi.service.sendCommentOnError;
...
Omadi.service.getUpdatedCommentJSON = function() {"use strict";
    /*jslint eqeq:true,plusplus:true*/

    var db, result, obj, nid, tid, nids, comment, cid, instances, field_name, i, v_result, output, hasComment;
    ...
    try {
        ...
        result = db.execute("SELECT * FROM comment WHERE cid < 0 LIMIT 1");
        
        if (result.isValidRow()) {
            cid = result.fieldByName('nid');
        ...
        Omadi.service.sendErrorReport("Exception in JSON comment creation: " + ex);
        ...
        try {
            ...
            Ti.API.error("db would not close");
            Omadi.service.sendErrorReport("DB WOULD NOT CLOSE in COMMENT");
        }
    }
    ...
    try{
        ...
        Omadi.service.sendErrorReport("Error stringifying comment obj: " + jsonEx);
    }
