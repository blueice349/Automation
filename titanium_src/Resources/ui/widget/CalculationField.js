...
function CalculationFieldWidget(formObj, instance, fieldViewWrapper){"use strict";
    ...
    // Reinitialize whenever a new field is created so the persistent object doesn't get tainted with other form types or nodes
    CalculatedFieldCache = {};
...
CalculationFieldWidget.prototype.getFieldView = function(){"use strict";
...
CalculationFieldWidget.prototype.redraw = function(){"use strict";
...
CalculationFieldWidget.prototype.getNewElement = function(index){"use strict";
    var settings, widgetView, dbValue, origValue, calculationTableView, recalculateButton;
...
CalculationFieldWidget.prototype.getRowValues = function(){"use strict";
    ...
    if (this.instance.settings.calculation.items != null && !this.instance.disabled) {
        row_values = [];
        if (this.instance.value != null && this.instance.value != "") {
            cached_final_value = this.instance.value;
        }
        else {
            cached_final_value = 0;
        }
        
        if(typeof this.instance.settings.calculation.items !== 'undefined'){
            
            if(Omadi.utils.isArray(this.instance.settings.calculation.items)){
                // Only sort if the items is an array
                // We can still support a key/value object, but sort will not work
                this.instance.settings.calculation.items = this.instance.settings.calculation.items.sort(Omadi.utils.sortByWeight);
            }
        
            for (idx in this.instance.settings.calculation.items) {
                if(this.instance.settings.calculation.items.hasOwnProperty(idx)){
                    calculation_row = this.instance.settings.calculation.items[idx];
                    value = 0;
                    ...
                    if (typeof calculation_row.field_name_1 !== 'undefined' && 
                        ...
                        instances[calculation_row.field_name_1].type == 'calculation_field') {
                            ...
                            required_instance_final_values = this.getRowValues(this.node, instances[calculation_row.field_name_1]);
                    ...
                    if (calculation_row.criteria != null && calculation_row.criteria.search_criteria != null) {
                        ...
                        if (!list_search_node_matches_search_criteria(this.node, calculation_row.criteria)) {
                            zero = true;
                        }
                    }
        ...
        if(typeof this.instance.settings.rounding !== 'undefined'){
            if(this.instance.settings.rounding == 'up'){
                final_value = Math.ceil(final_value);
            }
            else if(this.instance.settings.rounding == 'down'){
                final_value = Math.floor(final_value);
            }
            else if(this.instance.settings.rounding == 'integer'){
