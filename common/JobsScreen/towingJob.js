...
var towingPlus = false;

describe( 'Start Towing Job Process'.green, function () {

	it( 'Should updateStatus to towing from the jobs screen.'.green, function ( done ) {
			...
			if ( lastUser.userRole != 'client' && lastUser.userRole != 'AdminClient' ) {
				...
				.then( function ( openJobs ) {
					...
					if ( openJobs ) {
						...
						.click()
						.sleep ( 800 )
						.elementByNameIfExists( elements.jobsScreen.updateStatusOptions.towing )
						//.isDisplayed()
						.then( function ( towing ) {

							if ( towing ) {
								console.log( 'Towing the Job'.red );
								return towing
								.click();

							} else {
								return driver
								.elementByNameIfExists( elements.jobsScreen.updateStatusOptions.towingPlus )
								//.isDisplayed()
								.then( function ( towingPlus ) {
									...
									if ( towingPlus ) {
										console.log( 'Towing the Job +'.red );
										return driver
										.elementByName( elements.jobsScreen.updateStatusOptions.towingPlus )
										.click()
										.sleep ( 1000 )
										...
										console.log( 'No Towing Job Status'.red );
										return driver
		...
		.then( function() {
			...
			console.log( 'updateStatus To Towing Vehicle has Completed....'.green );
